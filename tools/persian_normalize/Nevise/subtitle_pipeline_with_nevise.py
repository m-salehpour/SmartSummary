...
def _load_nevise(vocab_path: Path, ckpt_path: Path, device: str):
    ...
    print(f"[Nevise] Loaded SubwordBert model with {get_model_nparams(model):,} parameters")
...
def _nevise_correct(model: SubwordBert, vocab: dict, device: str,
                   hazm_norm: Normalizer, text: str) -> str:
    print(f"\n[Nevise] Original sentence: {text!r}")
    sub_sents, _ = get_sentences_splitters(text)
    sub_sents = [
        hazm_norm.normalize(utils.space_special_chars(s))
        for s in sub_sents if s.strip()
    ]
    if not sub_sents:
    ...
    fixed = []
    for batch_labels, batch_sentences in batch_iter([(s,s) for s in sub_sents], batch_size=8, shuffle=False):
        print(f"[Nevise] Processing batch: {batch_sentences}")
        b_lbl, b_sent, bert_inp, bert_split = bert_tokenize_for_valid_examples(batch_labels, batch_sentences)
        if not b_lbl:
...
class NeviseCorrector:
    ...
    def clean(self, sentence: str) -> str:
        return _nevise_correct(self.model, self.vocab, self.device, self.hazm_norm, sentence)
...
class MixtralTranslator:
    ...
    def __init__(self, model_tag: str = "mixtral:instruct", host: str = "http://localhost:11434"):
        self.client    = AsyncClient(host=host)
        self.model_tag = model_tag
        print(f"[Translator] Initialized Ollama client for model '{model_tag}' at {host}")
    ...
    async def translate(self, segment: Dict) -> Dict:
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            ...
            if txt.startswith("{") and not txt.endswith("}"):
                raw = raw.rstrip() + "}"
                print(f"[Translator] ⚠️ Appended closing '}}'")
            ...
            if attempt < self.MAX_RETRIES:
                ...
                print(f"[Translator] ❌ All {self.MAX_RETRIES} attempts failed; skipping segment {idx}")
...
def run_pipeline(transcript_path: Path, output_srt: Path,
                 nevise_ckpt: Path, vocab_path: Path):
