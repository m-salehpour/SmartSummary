...
def continue_if_cut_ollama(client, model: str, partial_fa: str) -> str:
    ...
    cont_msgs = [
        ...
        {"role": "user", "content": "ادامهٔ ترجمه از همان‌جا. متن تکراری ننویس. فقط فارسی."}
    ]
    more = client.chat(model=model, messages=cont_msgs, options={"num_predict": 512})["message"]["content"]
...
# -------------- CLI --------------
def main():
    ap = argparse.ArgumentParser(description="English → Persian translator (Ollama + NLLB fallback).")
    ...
    ap.add_argument("--engine", choices=["ollama", "nllb", "hybrid"], default="hybrid",
    ...
    ap.add_argument("--host", type=str, default=DEFAULT_OLLAMA_HOST,
