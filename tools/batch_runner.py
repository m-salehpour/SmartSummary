...
def pair_audio_and_refs(audios: List[Path], refs: List[Path], fuzzy_threshold: float = 0.75) -> List[Tuple[Path, Path]]:
    """
    ...
    for a in sorted(audios):
        ...
        best = difflib.get_close_matches(a_norm, candidates, n=1, cutoff=fuzzy_threshold)
        if best:
            best_norm = best[0]
            ...
            logger.warning(f"ℹ️  Using fuzzy match for audio '{a.name}' → ref '{chosen.name}' (key='{best_norm}')")
            pairs.append((a, chosen))
        else:
...
def _find_pairs(
    ...
    """
    folder = Path(folder)
    audios = [p for p in folder.iterdir() if p.suffix.lower() in audio_exts and p.is_file()]
...
def _ensure_16k(audio_path: Path) -> Path:
    ...
    """
    try:
        ch, sr, dur = audio_info(str(audio_path))  # your utility: returns (channels, sample_rate, duration_sec)
