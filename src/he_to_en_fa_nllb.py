...
def _unprotect_with_regex(text: str, code_to_human: Dict[str, str]) -> str:
    for code, human in sorted(
        code_to_human.items(), key=lambda kv: len(kv[0]), reverse=True
    ):
...
def post_edit_en(text: str) -> str:
    fixes = [
        ...
        (
            r"\btook the Knesset oath and refused\b",
            "boycotted the Knesset swearing-in and refused",
        ),
