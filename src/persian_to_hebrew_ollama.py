...
gemma_opts = {
    ...
    "stop": [
        ...
        "בסדר גמור",
        "אנא ספק",
        "אני מקווה",  # kill small-talk/boilerplate
    ],
}
...
SMALLTALK = re.compile(
    r"(מה שלומך|אני מקווה|בסדר גמור|אנא ספק|לא יכול.?ה לעזור|ננסה שוב)", re.I
)
...
def looks_bad(he: str) -> bool:
    if len(he) < 24:
        return True
    if RE_ARABIC.search(he):
        return True
    if SMALLTALK.search(he) or META.search(he):
        return True
...
def sanitize_meta(he: str) -> str:
    ...
    lines = [
        ln for ln in he.splitlines() if not SMALLTALK.search(ln) and not META.search(ln)
    ]
    he = " ".join(lines)
...
def translate_chunk_gemma(client, model, fa_chunk, base_opts):
    messages = [
        {
            "role": "system",
            "content": "You are a Persian→Hebrew translator. Output HEBREW ONLY (U+05D0–U+05EA), no niqqud. Do not chat.",
        },
        {"role": "user", "content": "Translate to Hebrew only:\n\n" + fa_chunk},
    ]
    opts = {**base_opts}
    out = client.chat(model=model, messages=messages, options=opts)["message"][
        "content"
    ].strip()
    ...
    if looks_bad(he):
        ...
        retry_opts = {
            **opts,
            ...
            "top_p": 0.70,
            "repeat_penalty": 1.55,
            "seed": opts.get("seed", 0) + 1,
        }
        out2 = client.chat(model=model, messages=messages, options=retry_opts)[
            "message"
        ]["content"].strip()
...
_tok = AutoTokenizer.from_pretrained(
    _MODEL_ID, src_lang=_SRC
)  # keep use_fast default (True)
...
def translate_chunk(client: Client, model: str, fa_chunk: str, options: dict) -> str:
    ...
    while not looks_complete(he) and tries < 2:
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": "המשך את התרגום מהנקודה שבה הפסקת. "
                "אל תחזור על טקסט קודם. עברית בלבד, בלי ניקוד.",
            },
        ]
        more = client.chat(model=model, messages=messages, options=options)["message"][
            "content"
        ]
...
def translate_aya(client: Client, model: str, fa_text: str, options: dict) -> str:
    ...
    out = client.chat(model=model, messages=messages, options=aya_opts)["message"][
        "content"
    ].strip()
    ...
    if not is_hebrew_only(out) or out.startswith("שלום"):
        ...
        messages.append(
            {
                "role": "user",
                "content": "Output Hebrew only (U+05D0–U+05EA). Remove all Persian/Arabic letters and redo the translation. "
        ...
        out2 = client.chat(model=model, messages=messages, options=stricter)["message"][
            "content"
        ].strip()
...
def translate_chunk_aya(
    client: Client, model: str, fa_chunk: str, options: dict
) -> str:
    ...
    while not looks_complete(he) and tries < 2:
        ...
        cont_messages = [
            ...
            *AYA_FEWSHOT,
            ...
            {
                "role": "user",
                "content": "Continue the translation from where you stopped. "
        ...
        more = client.chat(model=model, messages=cont_messages, options=aya_opts)[
            "message"
        ]["content"].strip()
...
_RE_META = re.compile(
    r"(הנה תרגום לעברית|בסדר גמור|אנ[את] ספק|מה שלומך|אני מקווה|ננסה שוב)", re.I
)
...
_GLOSS.update(
    {
        ...
        # bad tail section
        "הם נצלו בהן מתוך אודות האל. הם נעשים עבדים בעונש עונה.": "יש בכך אותות לכוח האל ולעונש הכואב לפושעים.",
        # water/land bit
...
_PAT_FIXES = [
    (
        re.compile(r"הוא יעלה את השמים אל הארץ"),
        "הוא מנהל את ענייני העולם מן השמים אל הארץ",
    ),
    (
        re.compile(r"השמ[יי]ם מתכננים את דעתם אל הארץ"),
        "הוא מנהל את ענייני העולם מן השמים אל הארץ",
    ),
]
...
def translate_fa_to_he_chunked(
    text_fa: str, model: str, host: str, max_chars: int
) -> str:
