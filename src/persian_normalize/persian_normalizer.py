...
from src.persian_normalize.dictionary import (dictionary_mapping,
                                              fixator_dictionary)
    ...
    for special in special_list:
        ...
        for f in sorted(finder(special, text, False)):
            ...
            if len(text) >= (end + 1):
                if len(text) == (end + 1):
                    new_text, extra_pointer = substring_replace(
                        f"{zwnj}{special}", text, start + 1, end + 1, stripped=True
                    )
                    text = new_text
                    pointer += 1 + 1 - 1 - extra_pointer
                else:
