...
def model_inference(model, data, topk, DEVICE, BATCH_SIZE=16, vocab_=None):
    ...
    for batch_id, (batch_labels, batch_sentences) in tqdm(enumerate(data_iter)):
        ...
        for i, (a, b, c) in enumerate(
            zip(batch_clean_sentences, batch_corrupt_sentences, batch_predictions)
        ):
            results.append(
                {
                    "id": line_index + i,
...
def load_pretrained(model, checkpoint_path, optimizer=None, device="cuda"):
    ...
    max_dev_acc, argmax_dev_acc = (
        checkpoint_data["max_dev_acc"],
        checkpoint_data["argmax_dev_acc"],
    )
...
def spell_checking_on_sents(model, vocab, device, normalizer, txt):
    ...
    for t in test_data:
        print(t)
    greedy_results = model_inference(
        model, test_data, topk=1, DEVICE=device, BATCH_SIZE=1, vocab_=vocab
    )
    out = []
    ...
    for i, sent in enumerate(out):
        new_out.append(
            (
                utils.de_space_special_chars(out[i][0]),
                utils.de_space_special_chars(out[i][1]),
            )
        )
...
if __name__ == "__main__":
    ...
    normalizer = Normalizer(
        correct_spacing=True,  # spacing & punctuation
        persian_numbers=True,
        remove_diacritics=True,
    )
    vocab_path = os.path.join("model", "vocab.pkl")
    model_checkpoint_path = os.path.join("model", "model.pth.tar")
    model, vocab, device = load_pre_model(
        vocab_path=vocab_path, model_checkpoint_path=model_checkpoint_path
    )
    # test
