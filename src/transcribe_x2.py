...
from src.align import align_and_diarize
from src.audio_utils import audio_info, maybe_resample_to_16k
from src.batch_runner import run_batch_from_folder
# ─── 2) Your src ────────────────────────────────────────────────────────────
from src.profiler import profile_resources
    ...
    *,
    ...
    if resample_if_needed:
        try:
            ...
            logger.exception(
                "[run_experiment] optional resample check failed (continuing)."
            )
    ...
    with tqdm(
        total=2 if is_align_and_diarize_enabled else 1, desc="Pipeline", unit="step"
    ) as pbar:
        if is_asr_enabled:
        ...
        if is_align_and_diarize_enabled:
            ...
            try:
                ...
                logger.info(
                    "✅ Align+Diarize step completed in %.1fs", time.perf_counter() - t1
                )
                pbar.update(1)
            except Exception:
