...
def load_alignment_model(
    ...
    *,
    ...
    logger.info(
        f"[alignment] loading align model for lang={language_code!r} (cache={md})"
    )
...
def save_diarization_json(
    ...
    try:
        ...
        if diarization_json_path is None:
            ...
            diar_json_path = (
                (p / f"{audio_path.stem}.diarize.json") if p.is_dir() else p
            )
        ...
        try:
            ...
            # Case 3: list/iterable of dicts or mixed
                ...
                records = [
                    _coerce_record_like(r if isinstance(r, dict) else {"value": r})
...
def align_and_diarize(
    *,
    ...
    if verbose:
        ...
        logger.info(
            f"• device={device}  • return_char_alignments={return_char_alignments}"
        )
        logger.info(f"• do_diarize={do_diarize}  • parallel={parallel}")
        if do_diarize:
            logger.info(
                f"• diarization: min_speakers={min_speakers}  max_speakers={max_speakers}"
            )
    ...
    if do_diarize and parallel:
        ...
        try:
            ...
            logger.exception(
                "Failed to start background diarization; will fallback to sequential."
            )
            diar_future = None
    ...
    if do_diarize and diarization_result is not None:
        try:
            ...
            logger.exception(
                "⚠️  attach_speakers failed; continuing with alignment only."
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Align WhisperX segments (and optionally diarize)."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to ASR JSON (must include 'segments' & 'language')",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Cache dir for align models (offline)",
    )
    parser.add_argument("--char", action="store_true", help="Return char alignments")
    parser.add_argument(
        "--diarize",
        action="store_true",
        help="Also run diarization and attach speakers",
    )
    parser.add_argument(
        "--hf_token",
        type=str,
        default=None,
        help="HF token for diarization models (if required)",
    )
