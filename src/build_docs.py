...
import config
from src.compare_docs import (analyze_docs, compare_docs,
                              format_speaker_mapping_report,
                              guess_speaker_mapping, length_stats,
                              plot_per_speaker_counts_2, speaker_set_report)
from src.plots import (plot_hyp_durations_by_ref_bin_from_json,
...
def _collapse_turns(
    segments: List[Dict], speaker_key: str = "speaker"
) -> List[Tuple[str, str]]:
    """
...
def _find_pair(
    ...
    prefix: Optional[str],
    aligned_suffix="_aligned.json",
    diar_suffix=".diarize.json",
    ...
    if prefix:
        aligned = next(
            (p for p in folder.glob(f"{prefix}*{aligned_suffix}") if p.is_file()), None
        )
        diar = next(
            (p for p in folder.glob(f"{prefix}*{diar_suffix}") if p.is_file()), None
        )
    else:
        aligned = next(
            (p for p in folder.glob(f"*{aligned_suffix}") if p.is_file()), None
        )
        diar = next((p for p in folder.glob(f"*{diar_suffix}") if p.is_file()), None)
    if not aligned:
        raise FileNotFoundError(
            f"No aligned JSON matching '*{aligned_suffix}' found in {folder} (prefix={prefix!r})."
        )
    return aligned, diar
...
def generate_transcript_docx(
    ...
    *,
    ...
    # Output name
    out = Path(folder) / (
        out_name or f"{_default_title_from_prefix(prefix, aligned_path)}.docx"
    )
...
if __name__ == "__main__":
    out = generate_transcript_docx(
        folder=config.JSON_ASR_OUTPUT_DIR, prefix="Aridia Conference Call"
    )
    ...
    diar_json_path = "/src/asr_outputs/Aridia Conference Call.diarize.json"
    hyp_path = out
