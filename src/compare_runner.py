...
from src.normalizers import _run_llm_clean
...
def run_basic_comparisons(
    ...
    if print_hyp:
        print("\n[HYP RAW]\n", hyp_raw)
    print("\n=== RAW TRANSCRIPTION ===")
    metrics_raw = compare_texts(
        hyp_raw, ref_text, diff=diff, prefix_message=prefix_message
    )
...
def run_no_llm_clean_and_compare(
    ...
    if print_hyp:
        print("\n[HYP NO-LLM CLEAN]\n", hyp_no_llm)
    print("\n=== NO-LLM CLEANED ===")
    metrics_no_llm = compare_texts(
        hyp_no_llm, ref_text, diff=diff, prefix_message=prefix_message
    )
    return {
...
def run_llm_clean_from_raw(
    ...
    metrics_llm_raw = compare_texts(
        hyp_llm_raw, ref_text, diff=diff, prefix_message=prefix_message
    )
    return {
...
def run_llm_clean_from_no_llm(
    ...
    metrics_llm_no_llm = compare_texts(
        hyp_llm_no_llm, ref_text, diff=diff, prefix_message=prefix_message
    )
    return {
...
def _filter_by_script(text: str, script_hint: str | None) -> str:
    ...
    if _HAS_REGEX and script_hint.lower() in _SCRIPT_PROP:
        ...
        return _re.sub(rf"(?!{script_pat}|\p{{P}}|\p{{Zs}}).", "", text)
...
def load_reference_text(
    ...
    *,
    ...
    if extracted:
        ...
        logger.info(
            "Reference markers not found in %s; applying fallback='%s'.",
            path.name,
            fallback,
        )
        ...
        if fallback == "first":
            ...
            if not block:
                logger.warning(
                    "No block >= %d chars found; falling back to 'full'.", min_chars
                )
                text = cleaned
            else:
                text = block
...
def diff_word_level(ref: str, hyp: str) -> List[str]:
    ...
    return list(
        difflib.unified_diff(
            ref_words, hyp_words, fromfile="REFERENCE", tofile="HYPOTHESIS", lineterm=""
        )
    )
...
def compare_texts(
    ...
    logging.info(
        f"📊{prefix_message} WER: {metrics['wer']:.2%} | S={metrics['substitutions']} D={metrics['deletions']} I={metrics['insertions']}"
    )
    if diff:
