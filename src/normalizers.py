...
from src.lm_fa import get_persian_sp_kenlm
from src.persian_normalize.context_aware_normalizer import pipeline_clean
from src.persian_normalize.nevis import NeviseCorrector
from src.persian_normalize.persian_normalizer import persian_normalizer
from src.spellcheck_fa import build_symspell_fa
from src.utils import cleaned_filename
...
def cleaning(text: str, language: Optional[str] = None) -> Optional[str]:
    ...
    if language == "fa":  # Persian
        ...
        logger.info(
            f"len of text = {len(text)} len of s1 = {s1!r}, s2 = {s2!r}, s3 = {s3!r}"
        )
