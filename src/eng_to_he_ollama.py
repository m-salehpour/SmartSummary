...
def en_to_he_nllb(text_en: str, device: str = "cpu", max_new_tokens: int = 512) -> str:
    ...
    enc = _nllb_tok(text_en, return_tensors="pt", padding=False, truncation=True).to(
        device
    )
    ...
    out_tokens = _nllb_mdl.generate(
        **enc, forced_bos_token_id=bos_id, generation_config=gen_cfg
    )
    ...
    while not looks_complete(he) and tries < 2:
        ...
        cont = client.chat(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": "המשך את התרגום מן הנקודה המדויקת. "
...
def main():
    ap = argparse.ArgumentParser(
        description="Translate English ➔ Hebrew (Ollama small model + NLLB fallback)."
    )
    ...
    ap.add_argument(
        "--method",
        choices=["auto", "ollama", "nllb"],
        default="auto",
        help="Translation path. 'auto' tries Ollama then falls back to NLLB.",
    )
    ap.add_argument(
        "--ollama-model",
        default=OLLAMA_MODEL_TAG,
        help="Ollama model tag (for ollama/auto).",
    )
