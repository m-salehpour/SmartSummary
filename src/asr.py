...
import config
from config import HF_ROOT, LARGE_V3_MODEL
# keep your existing imports
from normalizers import cleaning as no_llm_clean
# NEW: import the orchestration helpers
from src.compare_runner import (run_basic_comparisons,
                                run_llm_clean_from_no_llm,
                                run_llm_clean_from_raw,
                                run_no_llm_clean_and_compare)
from src.utils import save_transcription_result
...
def evaluate_transcription(
    ...
    result = model.transcribe(
        audio, batch_size=batch_size, print_progress=print_progress
    )
    ...
    logger.info("hyp_no_llm_raw_json")
    hyp_no_llm_raw_json = save_transcription_result(
        no_llm_res, f"hyp_no_llm_{audio_path}", prefix="hyp_no_llm_raw_json"
    )
