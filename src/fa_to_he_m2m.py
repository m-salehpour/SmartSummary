...
@torch.no_grad()
def translate_chunk_fa_to_he(
    tok, model, device, fa_text: str, beams: int = 5, max_new_tokens: int = 512
) -> str:
...
def translate_chunk_fa_to_he_nllb(fa_text: str, max_new_tokens=512) -> str:
    _load_nllb()
    enc = _nllb_tok(fa_text, return_tensors="pt", padding=True, truncation=True).to(
        device
    )
...
def translate_fa_to_he(
    text_fa: str, max_chars: int = 900, beams: int = 4, max_new_tokens: int = 512
) -> str:
...
def main():
    ap = argparse.ArgumentParser(
        description="Translate Persian (fa) → Hebrew (he) with M2M100, chunked."
    )
    ap.add_argument(
        "--in", dest="inp", help="Input Persian file (UTF-8). Omit to read stdin."
    )
    ap.add_argument(
        "--max_chars", type=int, default=900, help="Max characters per chunk."
    )
    ap.add_argument("--beams", type=int, default=4, help="Beam size for generation.")
    ap.add_argument(
        "--max_new_tokens", type=int, default=512, help="Max new tokens per chunk."
    )
    args = ap.parse_args()

    text_fa = (
        open(args.inp, "r", encoding="utf-8").read() if args.inp else sys.stdin.read()
    )
