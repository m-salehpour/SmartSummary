...
def parse_docx_transcript(
    path: str | Path, language_hint: Optional[str] = None
) -> Transcript:
    """
...
def compare_docs(
    ref_docx: str | Path, hyp_docx: str | Path, exclude_speaker_names: bool = False
) -> Dict[str, Any]:
    """
...
def speaker_set_report(ref_docx: str | Path, hyp_docx: str | Path) -> Dict[str, Any]:
    """
...
def length_stats(ref_docx: str | Path, hyp_docx: str | Path) -> Dict[str, Any]:
    """
...
def analyze_docs(ref_docx: str | Path, hyp_docx: str | Path) -> Dict[str, Any]:
    """
...
def plot_per_speaker_counts_2(
    ref_docx: str | Path, hyp_docx: str | Path, save_path: Optional[str | Path] = None
) -> None:
    """
    ...
    speakers = sorted(
        set(list(ref_counts.keys()) + list(hyp_counts.keys())),
...
def _speaker_profiles(turns: List[Tuple[str, str]]) -> Dict[str, Dict[str, object]]:
    ...
    """
    prof: Dict[str, Dict[str, object]] = defaultdict(
        lambda: {"text": "", "count": 0, "chars": 0}
    )
...
def guess_speaker_mapping(
    ref_docx: Path, hyp_docx: Path, min_score: float = 0.35
) -> Dict[str, Any]:
    """
    Best-guess 1→1 mapping from hypothesis speakers to reference speakers.
    ...
    hyp_sorted = sorted(
        hyp_speakers, key=lambda s: int(hyp_prof[s]["chars"]), reverse=True
    )
    for h in hyp_sorted:
        best_r, best_score = None, -1.0
        for r in ref_speakers:
...
def format_speaker_mapping_report(result: Dict[str, Dict[str, object]]) -> str:
    ...
    if not result["mapping"]:
        ...
        items = sorted(
            result["mapping"].items(), key=lambda kv: kv[1]["score"], reverse=True
        )
