...
def _download_with_gdown(
    file_id: str, out_path: Path, sha256: str | None = None, max_retries: int = 3
):
    """
    ...
    if not _is_online():
        raise RuntimeError(
            f"Offline: cannot download {out_path.name}. Connect once to cache it."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "gdown is not installed and cannot be auto-installed while offline."
        )
    ...
    for attempt in range(1, max_retries + 1):
        try:
            log.info(
                f"Downloading {out_path.name} via gdown (attempt {attempt}/{max_retries}) …"
            )
            tmp = out_path.with_suffix(out_path.suffix + ".part")
            ...
            ret = gdown.download(
                id=file_id, output=str(tmp), quiet=False, fuzzy=True, use_cookies=False
            )
            if (
                not ret
                or not tmp.exists()
                or tmp.stat().st_size == 0
                or _looks_like_html_error(tmp)
            ):
                raise RuntimeError(
                    "gdown returned empty/HTML content (quota/private/bad id?)"
                )
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to download {out_path.name}: {e} (last gdown err: {last_err})"
        ) from e
